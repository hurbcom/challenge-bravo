...
export class FreeCurrencyApiService {
    ...
    public async GetUpdatedExchangeRateByCurrencyId(currencyId: string): Promise<number> {
        const response = await got(`${environment.externalServices.currencyApiUrl}?q=USD_${currencyId}&compact=ultra&apiKey=${environment.externalServices.currencyApiKey}`);
