<template>
  <v-app>
    <v-card outlined class="mx-auto" max-width="600">
      ...
      <div name="nav-bar">
        <v-bottom-navigation color="black">
          <v-btn @click="bottomNav='home'">
            <span>Início</span>
            <v-icon>mdi-home</v-icon>
          </v-btn>
          <v-btn @click="bottomNav = 'converter'">
            <span>Conversor</span>
            <v-icon color="yellow darken-1">mdi-transfer-right</v-icon>
          </v-btn>

          <v-btn @click="bottomNav = 'addCoin'">
            <span>Adicionar Moedas</span>
            <v-icon color="green darken-2">mdi-plus-circle-multiple</v-icon>
          </v-btn>

          <v-btn @click="bottomNav = 'deleteCoin'">
            <span>Deletar Moedas</span>
            <v-icon color="red darken-2">mdi-minus-circle-multiple</v-icon>
          </v-btn>
      ...
      <div name="home" v-if="bottomNav == 'home'">
        <home />
      </div>
    ...
    <v-snackbar v-model="snackbar.status" :color="snackbar.color" :timeout="snackbar.timeout">
      <div class="flex row justify-center">{{ snackbar.text }}</div>
    </v-snackbar>
...
export default {
  ...
  components: { converter, addCoin, deleteCoin, updateCoin, home },
