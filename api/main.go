...
import (
	"github.com/iiurydias/challenge-bravo/api/application"
...
func main() {
	...
	cliApp := &cli.App{
		...
		Usage: "an api that convert values of currencies",
	}
...
func runService(fileName string) error {
	var configParams *application.Config
	...
	api, err := application.New(configParams)
	if err != nil {
		return err
	}
	errChan := api.Run()
...
func waitForGracefullyShutdown(api *application.Application, errChan <-chan error) error {
...
func configSampleGenerator(flagFileName string) error {
	return errors.Wrap(application.NewConfigFile(flagFileName), "could not create a new config file")
}
