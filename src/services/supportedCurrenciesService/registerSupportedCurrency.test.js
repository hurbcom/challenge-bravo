...
import { RegisterSupportedCurrency } from './registerSupportedCurrency.js'
...
describe('Register Supported Currency Service', () => {
  ...
  test('should be able register an currency truthy in supported currencies', async () => {
    const registerSupportedCurrency = new RegisterSupportedCurrency(supportedCurrencyRepository, registerCurrencyService, resourceExtern)

    const code = 'MOCK'
    const response = await registerSupportedCurrency.execute(code)

    expect(response.includes(code)).toBeTruthy()
  })
