...
class CurrencyCache {
    ...
    public async getCurrencyExchangeRate(originalCurrency: string, finalCurrency: string): Promise<Big | null> {
        try {
            const exchangeRate = await this.getClient().get(
                this.getCurrencyExchangeRateKey(originalCurrency, finalCurrency)
            );
            ...
            for (const finalCurrency of Object.keys(exchangeRates)) {
                if (supportedCurrencies.includes(finalCurrency)) {
                    await this.getClient().set(
                        ...
                        'PX',
                        this.getCurrencyExchangeRateTTL()
                    );
    ...
    private getCurrencyExchangeRateTTL(): number {
        ...
        const utcExchangeRateExpirationTime = Date.UTC(
            now.getFullYear(),
            now.getMonth(),
            now.getDay(),
            now.getHours(),
